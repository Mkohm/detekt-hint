...
\newacronym{coh}{COH}{Composition over Inheritance}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{../images/demo.png}
    \caption{Screenshot of horizontal prototype - Showing the \gls{coh}/\gls{lsp} rule}
    \label{fig:liskov}
\end{figure}
...
The maintainer of Tusky\cite{tusky} ran the tool on the codebase using the \gls{cli} and did not find the tool particularly useful, and were generally concerned that the tool would generate too much noise. This may be a sign that the rules are weak and/or have too many false-positives. On the other hand, running the tool on the codebase using the \gls{cli} is not how the tool is supposed to work, and will defeat its purpose since it does not make use of the Github integration. Further communication with the maintainer of Tusky revealed that they currently did not have any linter set up for their build pipeline, and that a tool discovering possible bugs and crashes would be prioritized first. Later, a tool looking for design principle violations could be looked into.

\todo{Will add more feedback, when it ticks in}


Generally, it may seem like the tool generates too many false-positives, making most people concerned that the tool would generate too much noise. However, the rules that generates most of the noise may "destroy" the impression of the tool. Therefore, without any direct feedback on each of the single rules, the results from the open source community are inconclusive.     \todo{Try to get some feedback on the individual rules!!!}
 
\subsection{Internal evaluation}
\subsubsection{Rule invocations}
X number of popular open-source projects including one that were familiar to the writer were selected to evaluate the results of running detekt-hint. The rationale for selecting projects that were familiar to the writer were that false-positives easier could be detected, as one already have some insights into the architecture of those systems. The other systems were selected based on perceived quality (based on their popularity), and knowledge about the architecture of Android systems.

Following is a description of the open source kotlin projects that were selected for analysis: \textbf{Detekt} is the mother repository of the detekt-hint detekt plugin. Is is a project consisting all the source code for the detekt linter.   

\textbf{Ktor} is a


\textbf{iosched} is an Android app from google, that is supposed to show best practices in Android development. It is believed that this app is well architecturally well designed and does not contain much design issues. 


The analysis were run with all rules enabled, default configuration and with a \gls{lcom} threshold of 0.8. To be able to compare the results of different code-bases, any effort in specific configuration of individual rules for the different repositories were not done. This may result with a slightly higher rate of invocations\todo{use invocations/reports/detections consistently}. For example detekt will get a higher rate of invocations for the composition over inheritance because it consists of a number of rules, that all inherits from a common Rule class. Ignoring the Rule class from this check would have lowered the amount of invocations significantly. Because The results is presented in the table below. It is important to notice that even the high number of invocations it will not reflect the actual number of comments that will appear on a \gls{pr}. Comments on PR will only appear on the modified files.


\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{images/distribution.png}
    \caption{Number of detections per 1000 \gls{lloc} for each rule. }
    \label{fig:distribution}
\end{figure}

\subsubsection{Usefulness and accuracy of rules}
\todo{Should i mention what should be improved here in the evaluation? or should it be in the discussion?}
Building on the detections presented above, determining the ratio of false-positives and the rate of false-negatives will determine usefulness of the tool. Unfortunately, due to the nature of design principles, it is hard to know if the detection is a true or false-positive, and if there is any false-negatives. The rules were having different accuracy, ranging from almost none, to accurate. Below follows an breakdown of each rule, and its possible accuracy by looking into the detections in the different code-bases. 
\todo{create a table with false-positive rate and false-negative rate? And say bad, good, medium}
\begin{itemize}

    \item \gls{coh}: Its value is considered high, and the impact of reporting a false-positive is low due to good comments that mentions the inheritance and the public \gls{api} of the superclass. The rate of false-positives is high due to reporting all occurrences of inheritance within a unique package identifier. Together with a medium rate of false-negatives caused by not reporting issues with inheritance from third party packages, the rule has bad accuracy \todo{ They could change and then one is tightly coupled to them.}.

    \item \gls{isp}: Its value considered high, and the impact of reporting a false-positive is low. The rate of false-positives is low, but the amount of false-negatives is high since this principle can be broken by adding new methods to interfaces without having multiple classes implement the interface. This gives the rule a medium accuracy.
    
    \item \gls{ocp}: Its value is considered high, and the impact of reporting a false-positive is low. The rate of false-positives is medium, and the rate of false-negatives is medium, giving the rule a medium accuracy.
    
    \item \gls{lcom}: Its value is considered medium. Due to the need of finding out what is causing the cohesion to be low, the impact of reporting a false-positive is medium. Because it is controlled by the threshold value there are a medium amount of both false-positives and false-negatives, giving the rule a medium accuracy. 
...
Automatically adding comments to pull requests works as a good way of exposing possible design flaws late in the development process. With informative comments that is having references to the code, possible false-positives are sorted out in a matter of seconds, and easily closed with the "resolve conversation" option. If determining whether the reported issue is a true or false-positive takes more than a few seconds it is likely that the developer did not consider the design principle during development. In that case the comment serves as a reminder and would be good for learning purposes that is constructive for further development.
...
Needs more rules.
Needs to implement more constraints for rules so false-positive rate can be lowered. And a feedback mechanism.? Or should this be in the future work section. Maybe put evaluation and future work to same section?
...
\section{Discussion}
Given the results.. 
diskutere hvor bra det er, og hvordan det kunne vært gjort annerledes.

Discussion: compositoninheritance: Still, being invoked 99 times is much, but it is only a couple of times for each pull request, which can serve as a good checklist item to check to ensure good design. By implementing "do not check this supertype" in the config, the number of invocations will be drastically reduced. A possible better solution would report on all inheritance to reduce the rate of false-negatives to 0, and implementing a mechanism for "feedback" to the bot such that specified types could be ignored.
...
Adding up all the invocations, and using a \gls{pr} size of 500 lines of changed code we get an average of about 8.5 comments on each \gls{pr}, which is a quite large amount. However, this amount can be reduced drastically by configuring the composition over inheritance rule to ignore specified types. 
\section{Future work}
...
A full blown github app that could be installed on the repository. would require a backend service. Could be a good solution going forward with the tool, where configuration could be autoconfiged by reply options on the bot.
...
\subection{Github token not having write access to \gls{pr}'s}
