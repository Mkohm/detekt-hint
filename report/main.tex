...
When detekt is executed, it looks at all the rule-sets (and the configuration of them) and does analysis of the source code. When issues are found, the line number is noted, together with the comment to be added, and added to a report, which is the final output of the program. 

\subsection{Detekt-hint ruleset}
The Detekt-hint ruleset is a plugin to Detekt, and contains the implemented rules for \gls{ddpv}. The rules are described in detail below.
...
Detekt-hint is added to any repository on Github by using the pre-built Github Action called Detekt-hint docker action(? link). Using a .github workflow file, it executes the Github action, which again executes the analysis using the Detekt \gls{cli} using the detekt-hint ruleset. The output report (containing the comments) which is generated from the detekt analysis is then saved to a file. Danger is then executed and picks up the file with the comments, parses it and adds a comment on the github pull request for every comment in the file. A diagram of how everything interacts with each other can be seen in figure \ref{fig:integration}. 
